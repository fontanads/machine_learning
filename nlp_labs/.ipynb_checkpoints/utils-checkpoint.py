...
def my_word_tokenizer(s,tag_start='<s>',tag_end='</s>'):
    ...
    s_tokens = [tag_start] + [t.lower() for t in s_tokens if t.isalpha() or t == '.'] +  [tag_end]
    return s_tokens

def my_sentence_tokenizer(raw_text,tag_start='<s>',tag_end='</s>'):
    doc = raw_text.lower()
    doc = nltk.tokenize.sent_tokenize(doc)
    new_doc = []
    for s in doc:
        new_doc.append(my_word_tokenizer(s,tag_start,tag_end))
    return new_doc
